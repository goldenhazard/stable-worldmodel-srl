...
ENV_ACTION_DIM = 4
# Vendored: BASE_DIR is this file's own directory, so the sim config
# lookup at `{BASE_DIR}/conf/env/{object_name}.yaml` resolves to the
# vendored configs at .../stable_worldmodel/envs/deformable/conf/env/.
...
class FlexEnvWrapper(FlexEnv):
    def __init__(self, object_name):
        ...
        self._push_recording = False
        self._recorded_push_frames = []

    def start_push_recording(self):
        self._push_recording = True
        self._recorded_push_frames = []

    def pop_push_frames(self):
        frames = self._recorded_push_frames
        self._recorded_push_frames = []
        self._push_recording = False
        return frames
    ...
    def step_multiple(self, actions):
        ...
        for action in actions:
            ...
            if self._push_recording:
                for sub_img in imgs_list:
                    self._recorded_push_frames.append(
                        sub_img[self.camera_view][..., :3][..., ::-1]
                    )
